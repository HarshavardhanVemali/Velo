...
class RunService:
    ...
    def start_run(self, prompt: str) -> RunStateModel:
        ...
        self._start_background_execution(record.id, prompt)
        return self.store.get_run(record.id)

    def respond_to_run(self, run_id: str, response_text: str) -> RunStateModel:
        ...
        self._start_background_execution(run_id, merged_prompt)
        return self.store.get_run(run_id)

    def _start_background_execution(self, run_id: str, prompt: str) -> None:
        worker = Thread(target=self._execute_run, args=(run_id, prompt), daemon=True)
        worker.start()

    def _execute_run(self, run_id: str, prompt: str) -> None:
        try:
            result = run_agent_session(prompt, emit_event=lambda *args: self.store.append_event(run_id, *args))
